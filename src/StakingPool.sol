...
contract StakingPool is ReentrancyGuard, Ownable {
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return rewardPerTokenStored + ((lastTimeApplicable() - lastUpdateTime) * rewardRate * 1e18 / _totalSupply);
    }
    ...
    function getStakeInfo(address account)
        external
        view
        returns (uint256 stakedBalance, uint256 earnedRewards, uint256 rewardRatePerSecond, uint256 periodEnd)
    {
        return (_balances[account], earned(account), rewardRate, periodFinish);
    }
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner updateReward(address(0)) {
        ...
        require(rewardRate * rewardsDuration <= rewardToken.balanceOf(address(this)), "Insufficient reward balance");
